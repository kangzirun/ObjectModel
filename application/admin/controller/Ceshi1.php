...
class Ceshi1 extends Backend
{
    ...
    public function image_result()
    {
        ...
        if ($status === 'PROCESS_SUCCESS') {
            $this->view->assign("imageUrls", $json_data['imageUrls']);
        } else {
            $this->view->assign("imageUrls", []);
        }
        $this->view->assign("status", $status);
        $this->view->assign("errorMessage", $data->errorMessage);
    ...
    public function createimagetext()
    {
        ...
        // 场景二，使用任意可访问的url
        $file = fopen(cdnurl($params['refImageUrl'], "Http://127.0.0.1/"), 'rb');
        ...
        Log::info("德吉收手机 " . json_encode($result));
